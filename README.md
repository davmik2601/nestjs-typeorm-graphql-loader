...
❗️ ❗️ ❗️
-
Besides this lib solves Graphql n+1 (with Dataloader) -The specificity of this lib is also that it selects only the fields requested by graphql.
And you won't need to use `@ResolveFiled()` for relations.
But note that if you have custom fields that use other fields (from `@ResolveField()` or from other functions), those used other fields must start with  `_`  (ex: `_myField`) , otherwise similar custom fields will not work․
-
